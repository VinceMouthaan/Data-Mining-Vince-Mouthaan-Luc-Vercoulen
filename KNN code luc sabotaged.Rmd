...
```{r}
trainset_labels <- selectt(rawDF[1:384,], Outcome)
...
```{r}
cleanDF_test_pred <- knn(train = as.matrix(trainset), test = as.matrix(testset), cl= as.matrix(trainset_labels), k=10)
head(cleanDF_test_pred)
```
Now we made our predictive model. lets compare the outcome of the prediction with the labels of our test set. 
```{r}
confusionMatrix(cleanDF_test_pred, testtset_labels[[1]], positive = NULL, dnn = c("prediction", "TRUE"))
```
